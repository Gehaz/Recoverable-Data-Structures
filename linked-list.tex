...
\begin{procedure}[b]
	...
	retry: \While{\True}{ \label{find-outer-loop}
		...
		\While{\True}{
			succ := curr.next\;
			\eIf(\tcp*[f]{is succ was logically deleted?}) {curr.next is marked} {
	...
	\begin{procedure}[H]
		...
		\While{\True}{ \label{recover-insert}
			...
			\eIf{curr.key = key}{
				...
				\If{pred.next.\CAS (unmarked curr, unmarked new)}{
	...
	\begin{procedure}[H]
		...
		\While{\True}{ \label{recover-delete}
			...
			\eIf{curr.key $\neq$ key} {
				...
				\If(\tcp*[f]{logical delete}) {curr.next.\CAS (unmarked succ, marked succ)} { 
