...
Each process $p$ has a designated location in the memory, Announce[p]. Before trying to apply an operation, $p$ writes to Announce[p] the entire data needed to complete the operation. Upon recovery, $p$ can read Announce[p], and based on it to complete its pending operation, in case there is such. Formally, Announce[p] contains a pointer to a structure containing all the relevant data.
	...
	\begin{procedure}[H]
		...
		\While{\True}{ \label{recover-insert}
			...
			\eIf{curr.key == key}{
				...
				node.next = unmarked curr\;
				Announce[p] = new operation (Insert, pred, curr, new)\;
				\If{pred.next.\CAS (unmarked curr, unmarked node)}{
				%	Announce[p].type = InsertDone\;
					\KwRet \True\;
				}
			}		
		}
	...
	\begin{procedure}[H]
		...
		\While{\True}{ \label{recover-delete}
			...
			\eIf{curr.key != key}{
				...
				succ = curr.next\;
				Announe[p] = new operation (Delete, pred, curr, null)\;
				\If{curr.next.\CAS (unmarked succ, marked succ)}{ 
					curr.owner.\CAS (null, p)\; \label{recoverable-logical-delete}
					pred.next.\CAS (unmarked curr, unmarked succ)\;
					\If{curr.owner == p}{
						\KwRet \True\;
					}
...
\begin{procedure}[H]
	...
	\If{Announce[p].type = Insert}{
		\eIf{Announce[p].new is in the list $||$ Announce[p].curr.next is marked}{
			\KwRet \True\;
		}{
			\textbf{go to} \ref{recover-insert} \tcp*{restart Insert}\;
		}	
	}
	\If{Announce[p].type == Delete}{
		\eIf{Announce[p].curr.next is marked}{
...
For the Insert operation, $p$ tries to add the new node by performing a CAS. If it succeeded it will return true if there is no failure. In case of a failure after writing to Announce[p], upon recovery $p$ tries to complete its operation. If it already performed a successful CAS, that is, the node was added to the list, then either it is still in the list or that it was deleted. Therefore, if $p$ can find the new node in the list (using a similar procedure to find), or that it is marked, it must be that the node was added, and $p$ can return true. Otherwise, $p$ either crashed before performing the CAS, or that the CAS was unsuccessful. In both cases, the new node was not added, and $p$ can restart the Insert procedure.

For the delete operation, once $p$ logically delete a node $v$, it also tries to announce itself as the "removal" of the node by writing its id to owner using CAS. Assume a process $p$ crash while trying to delete the node. Upon recovery, if $p$ sees the node is not marked, then obliviously its deletion did not took affect, and it can restart the delete operation. However, if the node is marked, it might be that $p$ marked it before the crash, and it might be some different process trying to deleting the same node did so. As $p$ can not distinguish between the two, and since we desire for a lock-free implementation, we let $p$ to try and complete this deletion, even if it is not to process to logically delete the node. To avoid a scenario in which more then a single process "delete" the same node, they all compete for owner using CAS. The first one to perform it will win, and it is the only process to return true. It is easy to verify once a process wins the owner field, then eventually, if given enough time with no crash, it returns true, while any other process trying to delete the same node will have to retry the delete operation.
