		...
		\While{\True}{ \label{recover-delete}
			...
			\eIf{curr.key != key}{
				...
				\If{curr.next.\CAS (unmarked succ, marked succ)}{ 
					\textcolor{blue}{curr.deleter.\CAS (null, p)}\; \label{recoverable-logical-delete}
					pred.next.\CAS (unmarked curr, unmarked succ)\;
...
\begin{procedure}
	\caption{TryPush (Node newNode)}
	
	Node* oldTop = Top\;
	newNode.next = oldTop\;
	\eIf{Top.\CAS(oldTop, newNode)} {\KwRet \True} {\KwRet \False}
...
\begin{procedure}
	...
	Node *oldTop = Top\;
	Node *newTop\;
	\If{oldTop == NULL}{\KwRet EMPTY}
	newTop = oldTop.next\;
	\If{Top.\CAS(oldTop, newTop)}{\KwRet oldTop}
	\KwRet NULL\;
\end{procedure}

\begin{procedure}
	\caption{push (T myitem)}
	
	Node *nd = new Node (myitem)\;
	\While{\True}{
		\If{TryPush(nd)}{\KwRet \True}
		othervalue = visit(nd)\;
		\eIf{othervalue == NULL}{
			Record Success ()\;
			\KwRet \True\;
